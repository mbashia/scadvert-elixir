defmodule ScadvertWeb.ClientController do
  ...
  defp search_params(params) do
    ...
    query =
      from(u in User,
        where:
          fragment("? LIKE ?", u.firstname, ^"%#{params}%") or
            fragment("? LIKE ?", u.lastname, ^"%#{params}%") or
            fragment("? LIKE ?", u.email, ^"%#{params}%") or
            fragment("? LIKE ?", u.phone_number, ^"%#{params}%") or
            fragment("? LIKE ?", u.gender, ^"%#{params}%") or
            fragment("? LIKE ?", u.role, ^"%#{params}%") or
            fragment("? LIKE ?", u.status, ^"%#{params}%")
      )
